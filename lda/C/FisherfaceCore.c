...
//    end
    ...
    //temporary matrix used to store intermediate matrix calculations
    ...
    for (i = 1; i <= Class_number; i++)
    {
        ...
        for (k = 0; k < m->rows; k++)
        {
          tempMat = m[k][i] - m_PCA[k][0];
        }
        
        cblas_dgemm(CblasRowMajor, CblasNoTrans, CblasTrans, m->rows, 1, 1, 1,
                    *tempMat->data, tempMat->cols, *tempMat->data, tempMat->cols, 1, *Sb->data, Sb->cols);
        
    }
  
    matrix_destructor(tempMat);
