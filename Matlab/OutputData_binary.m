...
TestDatabasePath = 'C:\Users\Guru\Documents\Matlab\FCT Image Recognition\LDA Sept 08, 2009\Test2';
...
if(load_stuff == 0)
 ...
 fprintf('Writing Data Objects To Disk\n')
   ...
   [rows cols]=size(T);
   handle=fopen('T.mat','wb');
   %write the matrix size out first%
   %fprintf(handle,'%d %d\n',rows,cols);
   fwrite(handle,rows,'int');
   fwrite(handle,cols,'int');
   for (i=1:rows)
       for (j=1:cols)
           fwrite(handle,T(i,j),'double');
       end
   ...
   for (i=1:rows)
       for (j=1:cols)
           ...
           fwrite(handle,V_PCA(i,j),'double');
       end
       %fseek(handle,-1,'cof'); %move back 1 pos since there is a blank space there
       %fprintf(handle,'\n'); %replace that blank space with a newline
   end
   ...
   for (i=1:rows)
       for (j=1:cols)
           ...
           fwrite(handle,V_Fisher(i,j),'double');
       end
       %fseek(handle,-1,'cof'); %move back 1 pos since there is a blank space there
       %fprintf(handle,'\n'); %replace that blank space with a newline
   end
   ...
   handle=fopen('ProjectedImages_Fisher.mat','wb');
   %write the matrix size out first%
   %fprintf(handle,'%d %d\n',rows,cols);
   fwrite(handle,rows,'int');
   fwrite(handle,cols,'int');
   for (i=1:rows)
       for (j=1:cols)
           ...
           fwrite(handle,ProjectedImages_Fisher(i,j),'double');
       end
       %fseek(handle,-1,'cof'); %move back 1 pos since there is a blank space there
       %fprintf(handle,'\n'); %replace that blank space with a newline
   end
   ...
   handle=fopen('Inverse_V_Fisher.mat','wb');
   %write the matrix size out first%
   %fprintf(handle,'%d %d\n',rows,cols);
   fwrite(handle,rows,'int');
   fwrite(handle,cols,'int');
   for (i=1:rows)
       for (j=1:cols)
           ...
           fwrite(handle,Inverse_V_Fisher(i,j),'double');
       end
       %fseek(handle,-1,'cof'); %move back 1 pos since there is a blank space there
       %fprintf(handle,'\n'); %replace that blank space with a newline
   end
   ...
   handle=fopen('Inverse_V_PCA.mat','wb');
   %write the matrix size out first%
   %fprintf(handle,'%d %d\n',rows,cols);
   fwrite(handle,rows,'int');
   fwrite(handle,cols,'int');
   for (i=1:rows)
       for (j=1:cols)
           ...
           fwrite(handle,Inverse_V_PCA(i,j),'double');
       end
       %fseek(handle,-1,'cof'); %move back 1 pos since there is a blank space there
       %fprintf(handle,'\n'); %replace that blank space with a newline
   end
   ...
   handle=fopen('v_fisherT_x_v_pcaT.mat','wb');
   %write the matrix size out first%
   %fprintf(handle,'%d %d\n',rows,cols);
   fwrite(handle,rows,'int');
   fwrite(handle,cols,'int');
   for (i=1:rows)
       for (j=1:cols)
           ...
           fwrite(handle,v_fisherT_x_v_pcaT(i,j),'double');
       end
       %fseek(handle,-1,'cof'); %move back 1 pos since there is a blank space there
       %fprintf(handle,'\n'); %replace that blank space with a newline
   end
